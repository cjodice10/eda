...
#look at variable x1
cat_eda_subset = cat_eda[which(cat_eda$Variable=="cat_var1"),]
...
```{r, echo=TRUE, fig.show='hold'}
#now plot the event rate
ggplot(cat_eda_subset,aes(x=bin_id,y=EventRate)) +
  geom_bar(stat="identity",color="azure", fill="aquamarine3")+
  scale_x_discrete(name ="bin_id", 
                    limits= cat_eda_subset$bin_id)+
  theme_bw()+
  labs(title="Event Rate of Variable: x1")

#now plot the percent records
ggplot(cat_eda_subset,aes(x=bin_id,y=PctRecords)) +
  geom_bar(stat="identity",color="white", fill="steelblue")+
  scale_x_discrete(name ="bin_id", 
                    limits= cat_eda_subset$bin_id)+
  theme_bw()+
...
It should be noted that the EDA output dataframes for the numeric and categorical data are slightly different.  I did this for a reason and cannot remember why.  If this becomes an issue for users, I will go into the main code and make the necessary adjustments.
